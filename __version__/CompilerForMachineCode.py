def compile(instruction):
    ...
    #print([instruction, register_index, label])
    inst_ext_op = "".join(buffer)
    inst_ext_op = inst_ext_op.lower()
    #temp = list()

    #for i in inst_ext_op:
    #    if i.isalpha() == True:
    #        temp.append(i)
    ...
    if inst_type == 'r':
                    ...
                    if j in chars_in_reg:
                        if inst_ext_op == 'lui':
                            ...
                            if len(rs) < 3:
                                ...
                                imm.append(j)
           #print(rt,imm)
        elif inst_ext_op in itype_class2:
            flag1 = 0
            flag2 = 0
            for i in instruction:
                for j in i:
        ...
        for place in contents:
            if place== jumpaddr:
                jumpaddr= contents.index(jumpaddr)
                cnt=1
        if cnt!=1:
            print("wrong label") 
        f.close()
